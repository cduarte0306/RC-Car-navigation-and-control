 ...
 */
void VisionControls::processStereo(cv::Mat& stereoFrame, cv::Mat& pointCloudMat, std::pair<cv::Mat, cv::Mat>& stereoFramePair, cv::Matx44d& Q) {
    using namespace cv;
    ...
    if (disp16s.empty()) {
        stereoFrame.release();
        pointCloudMat.release();
        return;
    }

    if (disp16s.type() != CV_16S) {
